...
combineWidgets(ridge_copath,lasso_copath,nrow = 1,ncol = 2)
...
* We purposefully use the auto-selected lambdas from our prior modelling
```{r}
library(HDtweedie)
fit_ridge_cv_tweedie <- cv.HDtweedie(
  ...
  , p = 1.5
  , alpha = 0
  , lambda = fit_ridge$lambda/exp(4.5)
  , standardize=TRUE#
)
...
fit_lasso_cv_tweedie <- cv.HDtweedie(
  ...
  , p = 1.5
  , alpha = 1
  #, lambda = fit_lasso_cv$lambda/exp(2)
  , standardize=TRUE
)
```
...
plot(fit_ridge_cv_tweedie)
title('Ridge')
plot(fit_lasso_cv_tweedie)
title('Lasso')
```

## Compare Coef Paths

* We'll once again grab the minimum $\lambda$ model
```{r echo=FALSE}
